...
public class EjecutorJoda {
    ...
    private final Map<String, NodoAST.NodoMethod> metodosRegistrados = new HashMap<>();
    ...
    private void ejecutarNodo(NodoAST nodo) {
        ...
        if (nodo instanceof NodoAST.NodoDefine) {
            ...
            // *** NUEVO: registrar el metodo para poder invocarlo despues ***
            NodoAST.NodoMethod metodo = (NodoAST.NodoMethod) nodo;
            metodosRegistrados.put(metodo.nombre, metodo);
        } else if (nodo instanceof NodoAST.NodoLiteralCadena) {
            // *** NUEVO: detectar llamadas a metodo por el prefijo __llamada__ ***
            String val = ((NodoAST.NodoLiteralCadena) nodo).valor;
            if (val.startsWith("__llamada__")) {
                String nombreMetodo = val.replace("__llamada__", "");
                invocarMetodo(nombreMetodo);
            }
        }
    }

    // *** NUEVO: ejecuta el cuerpo del metodo registrado ***
    private void invocarMetodo(String nombre) {
        NodoAST.NodoMethod metodo = metodosRegistrados.get(nombre);
        if (metodo == null) {
            agregarSalida("[ERROR JVM-J] Metodo no encontrado: " + nombre);
            return;
        }
        ejecutarBloque(metodo.cuerpo);
    }
