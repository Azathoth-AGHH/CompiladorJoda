...
CAMBIO v2.2:
  - Se agrega ':' -> T_DOS_PUNTOS para la sintaxis correcta de case.
  - Se mantiene soporte de '++' y '--' como tokens unicos.
*/
public class AnalizadorLexico {
    ...
    private void procesarNumero() {
        ...
        while (!esFin() && esDigito(caracterActual())) {
            sb.append(caracterActual()); avanzar();
        }
        ...
        tokens.add(new Token(
            esDecimal ? Token.Tipo.T_LITERAL_DECIMAL : Token.Tipo.T_LITERAL_ENTERO,
    ...
    private void procesarSimbolo() {
        ...
        switch (c) {
            ...
            // ---- NUEVO ----
            case ':': tokens.add(new Token(Token.Tipo.T_DOS_PUNTOS,        ":",  lineaInicio)); break;
            ...
            case '&':
                if (!esFin() && caracterActual() == '&') {
                    ...
                    errores.add("Error lexico en linea " + lineaInicio + ": caracter '&' suelto. Se esperaba '&&'.");
    ...
    private char  caracterActual()  { return codigoFuente.charAt(posicion); }
    private char  siguienteCaracter() {
        return (posicion + 1 < codigoFuente.length()) ? codigoFuente.charAt(posicion + 1) : '\0';
    }
